...
2. **Override Layer**: Apply project-specific files from `.vendatta/agents/{agent}/`
...
overridePath := filepath.Join(".vendatta", "agents", agent, templatePath)
if fileExists(overridePath) {
    ...
    remotePath := filepath.Join(".vendatta", "remotes", remoteName, templatePath)
    if fileExists(remotePath) {
        return readFile(remotePath), true
    }

    // Fall back to base template
    basePath := filepath.Join(".vendatta", "templates", templatePath)
...
vendatta workspace create override-test

# Create custom rule override using modern .mdc format
mkdir -p .vendatta/agents/cursor/rules
cat > .vendatta/agents/cursor/rules/typescript.mdc << EOF
---
...
mkdir -p .vendatta/agents/cursor/commands
cat > .vendatta/agents/cursor/commands/pr-create.md << EOF
# Create Pull Request
...
# Create suppression
touch .vendatta/agents/cursor/rules/legacy.md  # Empty = suppress

# Generate configs
vendatta workspace create override-test

# Verify results
ls .vendatta/worktrees/override-test/.cursor/rules/
# Should contain: typescript.mdc (custom) but NOT legacy.md

ls .vendatta/worktrees/override-test/.cursor/commands/
# Should contain: pr-create.md (custom override)

cat .vendatta/worktrees/override-test/.cursor/rules/typescript.mdc
