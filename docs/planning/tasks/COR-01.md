...
1.  **Worktree Manager**:
    - Add `git worktree` at `.mochi/worktrees/[branch]`.
...
3.  **Env Injection**:
    ...
    - Inject `mochi_SERVICE_[NAME]_URL` into the `Exec` context for hooks.

## 🧪 Proof of Work
- Running `mochi dev branch-name` creates the worktree directory.
