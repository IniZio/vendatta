...
1.  **Worktree Manager**:
    - Add `git worktree` at `.nexus/worktrees/[branch]`.
...
3.  **Env Injection**:
    ...
    - Inject `nexus_SERVICE_[NAME]_URL` into the `Exec` context for hooks.

## 🧪 Proof of Work
- Running `nexus dev branch-name` creates the worktree directory.
