...
Service discovery environment variables (e.g., `nexus_SERVICE_WEB_URL`, `nexus_SERVICE_API_URL`) are not available in running containers, breaking a core advertised feature.
...
- **E2E Test Failure**: `TestnexusServiceDiscovery` fails because expected environment variables are missing
...
for name, svc := range cfg.Services {
    if svc.Port > 0 {
        ...
        if bindings, ok := json.NetworkSettings.Ports[nat.Port(pStr)]; ok && len(bindings) > 0 {
            ...
            env = append(env, fmt.Sprintf("nexus_SERVICE_%s_URL=%s", name, url))
        }
    }
}
```
...
After fix:
1. Run `TestnexusServiceDiscovery` - should pass
2. Manual verification: Create session with services, exec into container, check `env | grep nexus_SERVICE`
