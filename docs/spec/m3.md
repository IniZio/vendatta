...
**Correct Flow:**
```
User CLI → Coordination Server → [Transport: SSH/HTTP] → Node Agent → Provider (Docker/LXC/QEMU)
```

**NOT:**
```
User CLI → Provider with execRemote() → Remote Node
```
...
┌─────────────────────┐         ┌─────────────────────┐       ┌─────────────────────┐
│   Local Providers   │         │ Coordination Server │       │   Transport Layer   │
│   (All Types)       │         │                     │       │                     │
│                     │         │ • Node Management   │       │ • SSH Transport     │
│ • QEMU Local        │         │ • Command Dispatch  │       │ • HTTP Transport    │
│ • Docker Local      │         │ • Status Monitor    │       │ • Protocol Handling  │
│ • LXC Local         │         │ • SSH Proxy         │       │ ❌ Agent Communication│
│ ❌ Remote Support   │         │ ❌ Agent Coordination│       │                     │
└─────────────────────┘         └─────────────────────┘       └─────────────────────┘

❌ MISSING                      ✅ COMPLETED                   ✅ COMPLETED
┌─────────────────────┐         ┌─────────────────────┐       ┌─────────────────────┐
│   Node Agents       │         │ Configuration       │       │ Agent Integration   │
│                     │         │ System              │       │                     │
│ • Remote Execution  │         │ • YAML Config       │       │ • Rule Generation   │
│ • Provider Interface│         │ • Provider Config   │       │ • MCP Gateway       │
│ • Status Reporting  │         │ • Service Defs      │       │ • Context Supply    │
│ ❌ Agent Server     │         │                     │       │                     │
└─────────────────────┘         └─────────────────────┘       └─────────────────────┘
```
...
|-----------|--------|------------|-------------------|
| **Local Providers (All)** | ⚠️ Partial | 50% | QEMU has mixed responsibilities |
| **Configuration System** | ✅ Complete | 100% | Correctly implemented |
| **Agent Integration** | ✅ Complete | 100% | Correctly implemented |
| **Transport Layer** | ❌ Missing | 0% | SSH logic trapped in QEMU |
| **Coordination Server** | ❌ Missing | 0% | Critical missing component |
| **Node Agents** | ❌ Missing | 0% | Remote execution foundation |
| **SSH Key Handling** | ⚠️ Partial | 30% | Only basic generation exists |
| **Service Orchestration** | ⚠️ Partial | 40% | Local only, needs remote coordination |
| **Node Management CLI** | ❌ Missing | 0% | No interface to coordination server |
...
2. **Missing Layers**: Architecture has gaps between components
   - No transport layer → providers handle their own remote
   - No node agents → coordination server has no remote execution targets
   - No coordination server → no central management possible
...
### Phase 1: Foundation (Critical - 20% → 50%)
