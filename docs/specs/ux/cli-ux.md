# CLI UX Specification: Project nexus
...
| :--- | :--- | :--- |
| `init` | `nexus init` | Interactive prompts + Success checklist. |
| `workspace create <name>` | `nexus workspace create <branch>` | Progress bars for worktree creation & config generation. |
| `workspace up [name]` | `nexus workspace up [branch]` | Starts container + runs hooks + port forwarding. Blocks for logs unless `-d`. |
| `workspace shell [name]` | `nexus workspace shell [branch]` | Opens interactive shell in workspace container. |
| `workspace stop [name]` | `nexus workspace stop [branch]` | Stops container but preserves state. |
| `workspace down [name]` | `nexus workspace down [branch]` | Stops and removes container/network. |
| `workspace list` | `nexus workspace list` | Tabular data with color-coded status (Active=Green, Stopped=Yellow). |
| `workspace rm <name>` | `nexus workspace rm <branch>` | Deletes worktree and associated resources. |
| `config pull <url>` | `nexus config pull <url>` [--branch=branch] | Pulls shared templates/capabilities from Git repo. |
| `config sync <target>` | `nexus config sync <target>` | Syncs .nexus directory to configured remote target. |
| `config sync-all` | `nexus config sync-all` | Syncs .nexus to all configured remote targets. |
| `version update` | `nexus version update` | Updates CLI binary to latest version. |
| `internal mcp <id>` | `nexus internal mcp <session-id>` | Hidden: MCP server for AI agent tool execution. |
...
### **MCP Gateway**
- `nexus agent <session-id>` starts the MCP server for the session
