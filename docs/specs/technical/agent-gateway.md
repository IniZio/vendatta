...
### **Generation Process**
1. **Base Templates**: Start with built-in defaults from `.mochi/templates/`
2. **Remote Templates**: Merge with templates from `mochi config pull` sources
3. **Project Overrides**: Apply file-level overrides from `.mochi/agents/{agent}/`
...
### **Override Mechanism**
- **Override**: Place a file in `.mochi/agents/{agent}/rules/` or `skills/` to replace the base template
- **Suppression**: Create an empty file with the same name to prevent that rule/skill from being generated
- **Example**: Empty `.mochi/agents/cursor/rules/legacy-code.md` prevents legacy-code rule generation

### **File Resolution Priority**
1. **Project Override**: `.mochi/agents/cursor/rules/custom.md` (highest priority)
2. **Remote Template**: From `mochi config pull` sources
